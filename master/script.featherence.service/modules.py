...
def mode5(value, admin, name, printpoint):
	...
	if xbmc.getSkinDir() == skin and xbmc.getCondVisibility('System.HasAddon('+skin+')'):
		...
		#mode12()
		if xbmc.getInfoLabel('Skin.HasSetting(4K50)'): mode84("", admin, name, printpoint) #4K50
		mode215('_','','','')
...
def mode84(value, admin, name, printpoint):
	'''------------------------------
	---4K50--------------------------
	------------------------------'''
	...
	for line in output2:
		if "disconnected" in line: continue
			...
			if device != "": notification('Adding new mode 3840x2160p60!','Device found!' + space + str(device),'',3000)
			terminal('xrandr --newmode "3840x2160p60"  593.41  3840 4016 4104 4400  2160 2168 2178 2250 +hsync +vsync')
			terminal('xrandr --addmode '+device+' 3840x2160p60')
			if value == '1':
				returned = dialogyesno(str(name), 'Force 4K resolution now?', nolabel=localize(20428), yeslabel=addonString_servicefeatherence(32112).encode('utf-8'))
				if returned == 'ok': terminal('xrandr --output '+device+' --mode 3840x2160p60')
				else: terminal('xrandr --output '+device+' --mode 1920x1080 -r 59.94')
			else: pass
...
def mode218(value, admin, name, printpoint):
	...
	if "view" in value:
		...
		message = message + newline + "Window(home).Property(fanart)" + space2 + xbmc.getInfoLabel('Window(home).Property(fanart)')
		...
		if xbmc.getCondVisibility('Window.IsVisible(VideoFullScreen.xml)') and playerpaused:
			...
			message = message + newline + "custom4" + space2 + str(xbmc.getInfoLabel('$INFO[ListItem.Art(fanart)]')) #CUSTOM TEST
			message = message + newline + "custom5" + space2 + str(xbmc.getInfoLabel('$INFO[ListItem.IsParentFolder]')) #CUSTOM TEST
...
def mode231(value, value2, admin, name, printpoint):
	...
	if value2 == "": installaddonP(value, update=True)
