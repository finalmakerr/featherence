...
def addDir(name, url, mode, iconimage, desc, num, viewtype, fanart=""):
	...
	if '9' in printpoint: pass
		...
		liz.setArt({'poster': iconimage, 'fanart_image': fanart})
		
		fanart2 = setaddonFanart(fanart)
...
def setaddonFanart(fanart):
	...
	text = "fanart" + space2 + str(fanart) + extra
...
def clean_addonString(x, para="", para2=""):
	printpoint = "" ; c = x ; c2 = "" ; x_ = x  ; x2 = "" ; y = "" ; b = "" ; e = "" ; y_ = "" ; z = "" ; TypeError = "" ; extra = ""
	...
	if '"addonString' in c:
		...
		e = x.replace(x2,"",1)
		b = x2.replace('"addonString(',"") ; b = b.replace(')',"") ; b = b.replace(')',"") ; b = b.replace('"',"")
		try:
			y_ = int(b)
		...
		if 'addonString' in c:
			'''Double addonString (Music addon)'''	
			x2 = find_string(c, 'addonString(', ')"') ; c = c.replace(x2,"")
			e = x.replace(x2,"",1)
			b = x2.replace('addonString(',"") ; b = b.replace(')"',"") ; b = b.replace(')',"") ; b = b.replace('"',"")
			try:
				y_ = int(b)
		...
		x2 = find_string(x, 'addonString(', ')') ; c = c.replace(x2,"")
		e = x.replace(x2,"",1)
		b = x2.replace('addonString(',"") ; b = b.replace(')',"") ; b = b.replace(')',"") ; b = b.replace('"',"")
		try:
			y_ = int(b)
	...
	#c = '"' + c + '"'
	text = 'para' + space2 + str(para) + space + 'para2' + space2 + str(para2) + newline + \
...
def getAddonFanart(category=0, custom="", default="", urlcheck_=""):
	...
	if custom == "":
		if default == "":
			...
			returned = default
	elif "Custom_Playlist" in custom:
		if custom == "Custom_Playlist1": returned = Custom_Playlist1_Fanart
		elif custom == "Custom_Playlist2": returned = Custom_Playlist2_Fanart
		elif custom == "Custom_Playlist3": returned = Custom_Playlist3_Fanart
		elif custom == "Custom_Playlist4": returned = Custom_Playlist4_Fanart
		elif custom == "Custom_Playlist5": returned = Custom_Playlist5_Fanart
		elif custom == "Custom_Playlist6": returned = Custom_Playlist6_Fanart
		elif custom == "Custom_Playlist7": returned = Custom_Playlist7_Fanart
		elif custom == "Custom_Playlist8": returned = Custom_Playlist8_Fanart
		elif custom == "Custom_Playlist9": returned = Custom_Playlist9_Fanart
		elif custom == "Custom_Playlist10": returned = Custom_Playlist10_Fanart
	else:
		returned = custom
	...
	'Custom_Playlist1_Fanart' + space2 + str(Custom_Playlist1_Fanart)
...
def CATEGORIES_SEARCH(mode=3, name='-' + localize(137), url="", num=""):
	...
	addDir(name,url,mode,featherenceserviceicons_path + 'se.png',localize(137) + space + 'YouTube',num,"", "")
