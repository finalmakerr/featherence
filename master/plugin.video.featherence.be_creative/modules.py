...
def CATEGORIES():
	...
	addDir('-' + addonString(100).encode('utf-8'),'',100,featherenceserviceicons_path + 'star.png',addonString_servicefeatherence(32800).encode('utf-8'),'1',0,getAddonFanart(100, urlcheck_=True)) #My ?
	
	addDir('test','',101,featherenceserviceicons_path + 'music.png','description','1',0,getAddonFanart(101, default="http://p1.pichost.me/i/28/1509965.jpg", urlcheck_=True))
...
def CATEGORIES101(name, iconimage, desc, fanart):
	...
	addDir('test',list,17,'https://lh6.ggpht.com/remKtmFiZgZ3yP409xrHZNIIe9M_bV9xEdQM1IOkJw5Ep28lDGVnC7z9iqgv-PsTsHA=w300',"description",'1',0,getAddonFanart(background,custom="https://lh6.ggpht.com/remKtmFiZgZ3yP409xrHZNIIe9M_bV9xEdQM1IOkJw5Ep28lDGVnC7z9iqgv-PsTsHA=w300"))
	
	addon = 'plugin.audio.jango'
	thumb, fanart, summary, description, plot = getAddonInfo(addon)
	addDir('test2','plugin://'+addon,8,thumb,plot,addon,0,getAddonFanart(background, custom=fanart))